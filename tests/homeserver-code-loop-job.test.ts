...
import { mkdtempSync, writeFileSync, mkdirSync, existsSync, readFileSync, readdirSync, unlinkSync, rmSync, utimesSync, symlinkSync } from "node:fs";
...
describe("startCodeLoop — lifecycle + git diff harvest", () => {
  ...
  it("defaults to seeded-only writes and returns a machine-readable scope violation", async () => {
    const engineRun = async (sandboxDir: string): Promise<EngineRunResult> => {
      writeFileSync(join(sandboxDir, "a.ts"), "export const x = 2;\n");
    ...
    const start = await startCodeLoop(
      { instruction: "edit only a.ts", files: [{ path: "a.ts", content: "old\n" }] },
      startCfg(),
      fakeDeps({ engineRun }),
    );
  ...
  it("never returns generated bytecode/cache artifacts, even with a broad explicit scope", async () => {
    const engineRun = async (sandboxDir: string): Promise<EngineRunResult> => {
      mkdirSync(join(sandboxDir, "__pycache__"));
      writeFileSync(join(sandboxDir, "__pycache__", "module.cpython-312.pyc"), "binary\0cache\n");
      writeFileSync(join(sandboxDir, "safe.ts"), "export const safe = true;\n");
      return { outcome: "completed", usage: { turns: 1, wall_ms: 100, prompt_tokens: 1, completion_tokens: 1 }, finalMessage: "done", unparseableLines: 0, detail: "" };
    };
    const start = await startCodeLoop(
      { instruction: "write the safe file", files: [{ path: "seed.ts", content: "seed\n" }], writable: ["**/*"] },
      startCfg(),
      fakeDeps({ engineRun }),
    );
    if (!start.ok) throw new Error("start failed");
    ...
    const result = getJobResult(start.work_id);
    if (result.kind !== "result") throw new Error("missing result");
    expect(result.result.changed_files).toEqual(["safe.ts"]);
    expect(result.result.diff).toContain("safe.ts");
    expect(result.result.diff).not.toContain("__pycache__");
    expect(result.result.diff).not.toContain("module.cpython-312.pyc");
    expect(result.result.scope_violations).toContain("__pycache__/module.cpython-312.pyc");
  });

  it("rejects contradictory generated-artifact scope before admission", async () => {
    let executed = false;
    const result = await startCodeLoop(
      { instruction: "do it", files: [{ path: "a.ts", content: "x\n" }], writable: ["**/*.pyc"] },
      startCfg(),
      fakeDeps({ engineRun: async () => {
        executed = true;
  ...
  it("rejects a runtime symlink escape from the returned diff", async () => {
    const outside = mkdtempSync(join(tmpdir(), "cl-job-outside-"));
    const engineRun = async (sandboxDir: string): Promise<EngineRunResult> => {
      symlinkSync(outside, join(sandboxDir, "escape"));
      writeFileSync(join(sandboxDir, "escape", "outside.txt"), "must not escape\n");
      return { outcome: "completed", usage: { turns: 1, wall_ms: 100, prompt_tokens: 1, completion_tokens: 1 }, finalMessage: "done", unparseableLines: 0, detail: "" };
    };
    const start = await startCodeLoop(
      { instruction: "write inside", files: [{ path: "seed.ts", content: "seed\n" }], writable: ["escape/**"] },
      startCfg(),
      fakeDeps({ engineRun }),
    );
    if (!start.ok) throw new Error("start failed");
    ...
    expect(result.result.scope_violations).toContain("escape");
    expect(readFileSync(join(outside, "outside.txt"), "utf8")).toBe("must not escape\n");
  });
