# Design: `code_loop` — owner-agent sandboxed pi-driven agentic coding tool on M5
...
| From | Idea |
|---|---|
| native-driver | owner-tier + real-key + `scope∈{agent,admin}` gate; byte-identical unknown-tool error; protected-path exit-time re-verification; ≥9/10 gate-d acceptance discipline before default-on; closed-command-execution philosophy (applied to `check_cmd`) |
...
Three owner-agent MCP tools on the existing gateway (`POST /mcp`) — `code_loop_start`, `code_loop_status`, `code_loop_result` — manage an **async job** that wraps a **pi subprocess installed on the box**, pointed back at the gateway's own `/v1` (the **configured bind host** — the tailnet IP on the live box, never a hardcoded loopback) with a dedicated owner-tier, agent-scope service key. The harness materializes caller-supplied seed files into a throwaway sandbox with its **own `git init`** (the gate-d fixture-corruption fix), spawns pi inside an **OS cage** (systemd resource/network scope + bwrap filesystem view) under wall-clock/turn/token caps, then harvests `git diff` + pi's final message. The caller polls status and fetches the result — a diff, changed-file set, and summary — and applies the diff itself. **The box never mutates a live checkout.**
...
| Path | Change |
|---|---|
| `src/homeserver/mcp.ts` | `toolDefs()` gains a `principal` parameter; the three `code_loop_*` tools appear **only** for an owner-tier real key with `scope=agent\|admin`; `callTool()` re-checks the same guard and returns the **byte-identical unknown-tool error** to ineligible callers — invisible, not just forbidden |
...
3. Mint the service key with a **fresh timestamped alias** — `keys mint --alias code-loop-$(date +%Y%m%d-%H%M%S) --tier owner --scope agent` (`keys mint` REJECTS reused aliases even after revocation) — with model allow-list `[qwen3-coder-next-80b]` and TPM/daily quotas as a defense-in-depth backstop → `.env` as `HOMESERVER_CODE_LOOP_API_KEY`. A real owner-tier keystore key makes `owner_request_log` fire per turn; agent scope prevents operator-route access.
...
- `writable`: optional enforceable relative path/glob allowlist for returned changes. When omitted,
  the safe default is exact seeded files only. It is independent from `protected`, which remains
  reporting-only.
  Scope paths are canonicalized before admission; traversal, ambiguous separators, and contradictory
  generated-artifact scopes fail closed before any durable run claim.
...
- The returned diff is assembled only from canonical paths admitted by the enforceable `writable`
  scope (seeded-only when omitted). Generated bytecode/cache artifacts, symlink escapes, and the
  harness `.meta.json` are never returned; rejected paths are exposed as `scope_violations` and
  disqualify `check_cmd`.
...
|---|---|---|
| Owner-agent only | `principal.tier === "owner" && principal.keyHash !== null && principal.scope ∈ {"agent","admin"}`. Tier/key identity preserves the owner-content boundary; scope supplies route authority. Implicit-admin and legacy static keys remain excluded. | `mcp.ts` — both `toolDefs(principal)` filtering **and** `callTool()` dispatch |
...
| Writable result scope | Independent `writable` path/glob allowlist (seeded-only when omitted); canonical harvest filters reject out-of-scope paths, symlink escapes, and generated cache/bytecode before diff assembly and report `scope_violations` | `code-loop.ts` |
...
- **`pasta -T <forwardPort>`** (passt) — **network egress**: runs the child in a fresh user+net namespace with **no general outbound route** (all egress blocked) and forwards **only one loopback port** to the host loopback, where a per-run Node relay (`startGatewayRelay`) bridges to the real gateway (`HOMESERVER_HOST:PORT`). The caged pi's *only* reachable destination is the gateway callback, at `http://127.0.0.1:<forwardPort>/v1` in-namespace. No `--config-net` (which would grant general NAT egress). This is the change that makes egress-blocking actually hold on this box.
  - **The relay is a PATH-ALLOWLISTED HTTP forwarder, not a raw byte-pipe** (adversarial-review escalation, 2026-07-02). The service key now uses `tier=owner, scope=agent`, so the gateway itself denies admin routes; the relay remains mandatory defense in depth and forwards **only** `POST /v1/chat/completions`, `GET /v1/models` (read-only, content-blind), and `GET /healthz` (the self-test's unauthenticated liveness arm). **Every other method+path → `403 code_loop relay: path not allowed` without any upstream connection** (`homeserver_code_loop_relay_denied_total` increments), and non-HTTP/garbage closes the socket. Request + response stream both ways (SSE-safe). This keeps prompt-injected code from widening its network authority even if a credential is accidentally provisioned with broader scope.
...
- **Maintenance mode:** `start` refuses (`maintenance`) when an explicit bench/evaluation window is
  engaged (hook the same flag the gateway consults, `gateway.ts:2371-2372`).
...
6. **Owner-tier service key scope — resolved 2026-07-30.** Keep `tier=owner` for deliberate owner logging/admission, but mint or rotate the credential with `scope=agent`. This preserves `ask`/`code_loop` while denying `/admin/*` and `/ledger`; Claude, Codex, and the internal code-loop service receive separate rotation families.
...
| S4 | Owner-agent gate + byte-identical unknown-tool error | **Addressed** — §6 table; exact-bytes unit test (§11) |
