...
| T10 persistent prefix cache | **Partly existing; measurement implemented** | llama.cpp server supports RAM prompt cache/context checkpoints and the runner records cached tokens. Repository-session invalidation semantics remain unimplemented. Related context work: issue #126. |
