...
| T05 adaptive MTP | **Offline policy synthesis implemented; live matrix pending** | A content-blind synthesizer now consumes one direct report plus multiple speculative-depth reports and selects per workload/concurrency cell only when quality is non-inferior, acceptance is observable, and useful completions/minute clears an explicit margin; otherwise it fails closed to direct. This is not an online rolling controller. Exact direct/depth-1/depth-2/deeper traces across the intended concurrency matrix remain required. |
