...
- **Authenticated harness bases:** `eval "$(m5-auth --env)"` exports `M5_GATEWAY_URL` at the gateway root and `M5_OPENAI_BASE_URL` at that root plus `/v1`; it also retains `M5_BASE_URL` as an OpenAI-base compatibility alias. Public mode requires at least one of those base variables to be explicitly configured and fails before Keychain access when none is set; it never guesses a loopback public route. The helper also refuses empty or inconsistent overrides before reading the token: after trailing-slash normalization, the OpenAI base must equal the gateway root plus `/v1`. Compose routes exactly as `"$M5_GATEWAY_URL/delegate"`, `"$M5_GATEWAY_URL/ledger"`, and `"$M5_OPENAI_BASE_URL/chat/completions"`. Do not append `/delegate` or `/ledger` to either OpenAI-base variable: that produces the non-route `/v1/delegate` or `/v1/ledger`. `--tailnet` changes both bases to the authenticated tailnet gateway, never to loopback. Before a consequential call, `m5-auth --check` (or `m5-auth --check --tailnet`) performs a non-emitting authenticated preflight; see the rotation and recovery runbook in [`../deploy/README.md`](../deploy/README.md#credential-preflight-and-owner-recovery).
...
| Scope | `monitor` | Read-only `GET /healthz`, `/models*`, `/metrics`, `/ledger*`, and `/ops/summary`; no inference or mutation. |
...
requested value is resolved the way ingress resolves it — **trimmed, not case-folded** (#80) — and
the response is keyed by that value: `?taskType=review-bounded%20` reports the real `review-bounded`
lane instead of the generic fallback, while a case variant such as `?taskType=Review-Bounded` is
reported as frontier-only, because routing and the evidence ledger really do treat it as a separate
bucket.
...
Guest, owner-agent, identity-less static admin, and the implicit admin path all report
`available:false` with a stable `availabilityReason` enum:
`requires_owner_admin`, `requires_minted_owner_admin`, or `missing_reviewer_identity`.
Monitor-scope keys do not reach this endpoint at all: the global read-only monitor fence returns
`403 route_not_allowed` before review-lane dispatch. This endpoint does not claim any model output
is ground truth.
...
Minted owner-admin, exact-ledger-id writer for verified `review-bounded` delegations only.
`recordReviewerUsefulness()` owns the row-level narrowing and atomic write-once decision
(`review-bounded`, verifier present, exact retry vs conflict). The route adds auth, content-type /
JSON validation, reviewer-identity binding from the authenticated logical alias, bounded
content-blind `notes`, and the HTTP error envelope. Static env admin keys, owner-monitor keys, and
the implicit no-key loopback admin path are deliberately excluded; the caller must present a
minted owner-tier admin credential with a non-empty logical alias.
...
```json
{
  "error": {
    ...
    "param": "ledgerId",
    "message": "Reviewer usefulness is already recorded differently (existing usefulness='pass', reviewer='reviewer-admin', ts='<RFC3339 UTC>', notesPresent=true, noteChars=36); attempted usefulness='redo', reviewer='reviewer-admin', notesPresent=false, noteChars=0. Exact retries are idempotent; differing overwrites are rejected."
  },
  "conflict": {
...
partially overwrite or double-record the reviewer-usefulness columns. The content-blind
`request_log` row is written afterward, best-effort, in a separate durability path.
