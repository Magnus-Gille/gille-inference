...
| `GET /v1/capabilities/review-lane` | user | **Review-lane preflight (#74).** No-I/O, content-blind lookup of which lane a review task TYPE will get — `code-review` is always `frontier-only` (unchanged); `review-bounded` (the three bounded subtasks: classify-findings / detect-anti-pattern / verify-output-shape) is `local-advisory` and reports `advisoryOnly`/`promoted`. The response also advertises whether reviewer-usefulness recording is available, without treating it as ground truth. Lets an orchestrator decide before spending a `/delegate` round trip, so it never waits on a local review result that will not come. See `docs/review-bounded-lane.md`. |
...
| `PUT /ledger/{id}/reviewer-usefulness` | **admin** | Owner-authenticated write for one validated `review-bounded` row by exact `ledgerId` (#112). Uses the authenticated logical alias as reviewer identity, accepts only closed `pass|partial|redo|wrong`, keeps notes bounded/content-blind, makes exact retries idempotent, and rejects differing overwrites with `409 reviewer_usefulness_conflict`. |
