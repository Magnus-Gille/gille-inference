...
| `GET /v1/capabilities/review-lane` | user | **Review-lane preflight (#74).** No-I/O, content-blind lookup of which lane a review task TYPE will get — `code-review` is always `frontier-only` (unchanged); `review-bounded` (the three bounded subtasks: classify-findings / detect-anti-pattern / verify-output-shape) is `local-advisory` and reports `advisoryOnly`/`promoted`. Lets an orchestrator decide before spending a `/delegate` round trip, so it never waits on a local review result that will not come. See `docs/review-bounded-lane.md`. |
...
- `ask` — `{model, prompt, system?, max_tokens?, temperature?, top_p?, top_k?, min_p?, delegator_model_id?, files?}` → runs a completion on the chosen
  ...
  `structuredContent {model,text,finish_reason,truncated,usage}`. `usage` is content-blind:
  prompt/input, completion/output, total, reasoning, and cache counters are surfaced when the
  backend reports them and stay `null` otherwise. A model outside the key's allow-list, an
  exhausted credit budget, a quota hit, or a busy box all map to `isError:true` with a clear
  message (never a thrown JSON-RPC fault). A token-limit finish (`finish_reason="length"`) also
  returns `isError:true`, with the same structured payload preserved, so callers can retry with a
  higher `max_tokens` instead of accepting a silent empty/partial answer. `max_tokens` uses the
  same fleet/per-model ceiling as raw chat. `delegator_model_id` is optional telemetry:
