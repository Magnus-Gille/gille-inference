...
| `POST /v1/roster-proposals` | minted `service:hugin` owner | Content-blind roster-proposal validation and durable admission only. No apply/re-arm/widen/model-admin operation. Production rejects until all server-owned roster, restore, template, and canary registries are configured; see `docs/roster-proposal-contract.md`. |
